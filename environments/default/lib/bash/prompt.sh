...
function __promptline_ps1 {
  ...
  # section "a" slices
  __promptline_wrapper "$(if [[ -n ${ZSH_VERSION-} ]]; then print %m; elif [[ -n ${FISH_VERSION-} ]]; then hostname -s; else printf "⦿" \\h; fi )" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; is_prompt_empty=0; }
  __promptline_wrapper "$(if [[ -n ${ZSH_VERSION-} ]]; then print %n; elif [[ -n ${FISH_VERSION-} ]]; then printf "%s" "$USER"; else printf "♙" \\u; fi )" "$slice_prefix" "$slice_suffix" && { slice_prefix="$slice_joiner"; is_prompt_empty=0; }
...
function __promptline {
  ...
  local d_fg="${wrap}38;5;232${end_wrap}"
  local d_bg="${wrap}48;5;172${end_wrap}"
  local d_sep_fg="${wrap}38;5;172${end_wrap}"
